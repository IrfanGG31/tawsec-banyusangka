...
import {
  ...
  Upload, Image as ImageIcon, Loader2, Star, Settings, Trophy,
  Eye, EyeOff, ExternalLink
} from "lucide-react";
import SetupTab from "@/components/admin/SetupTab";

type TabType = "progress" | "updates" | "dampak" | "dokumentasi" | "setup" | "submissions";
...
export default function AdminDashboardPage() {
  ...
  const fetchAllTables = async (supabase: ReturnType<typeof createClient>) => {
    ...
    const [progRes, updRes, dampRes, dokRes, subRes] = await Promise.all([
      ...
      <header className="bg-slate-900 border-b border-slate-800 sticky top-0 z-30 shadow-md">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-sky-500 to-indigo-600 flex items-center justify-center text-white shadow-md font-bold">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 pt-8">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4 mb-8 bg-slate-900 border border-slate-800 p-2 rounded-2xl">
          ...
          <div className="flex items-center gap-2">
            ...
            {activeTab !== "setup" && activeTab !== "submissions" && (
              <button
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
            <div className="p-5 border-b border-slate-800 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
              <div>
                <h2 className="font-serif font-bold text-white text-lg flex items-center gap-2">
                  ...
                  Submission Challenge — Live Showcase Wall
                </h2>
                <p className="text-slate-400 text-xs mt-0.5">
                  Moderasi karya peserta. Toggle visibilitas atau hapus submission.
                </p>
              </div>
              <div className="flex items-center gap-2">
            ...
            {submissionsList.length === 0 ? (
              <div className="p-12 text-center">
                ...
                <p className="text-slate-500 text-sm">Belum ada submission challenge.</p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-slate-800/70 text-slate-400 uppercase tracking-wider">
                      <th className="text-left px-4 py-3 font-semibold">Foto</th>
                      <th className="text-left px-4 py-3 font-semibold">Tim</th>
                      ...
                      <tr key={sub.id} className="hover:bg-slate-800/40 transition-colors">
                        <td className="px-4 py-3">
                          <div className="w-12 h-12 rounded-xl overflow-hidden bg-slate-800 border border-slate-700">
                            <img src={sub.foto_bukti_url} alt={sub.nama_tim} className="w-full h-full object-cover" />
                          </div>
                        </td>
                        <td className="px-4 py-3">
                          <span className="text-white font-bold">{sub.nama_tim}</span>
                          {sub.caption_singkat && (
                            <p className="text-slate-500 text-[10px] mt-0.5 line-clamp-1">{sub.caption_singkat}</p>
                          )}
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <td className="px-4 py-3">
                          <span className={`px-2.5 py-1 rounded-lg text-[10px] font-bold ${
                            ...
                            {sub.status === 'tampil' ? '✓ Tampil' : '✕ Disembunyikan'}
                          </span>
                        </td>
                        <td className="px-4 py-3 text-slate-500">
                          {new Date(sub.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-end gap-2">
                            <button
                            ...
                            <button
                              ...
                              title={sub.status === 'tampil' ? 'Sembunyikan' : 'Tampilkan'}
                            >
                              {sub.status === 'tampil' ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
                            </button>
                            <button
                              onClick={async () => {
